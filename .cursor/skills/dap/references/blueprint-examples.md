...
> - The orchestrator logically merges imported YAML files at the TOSCA level — but YAML parsing is per-file. YAML anchors and aliases must be in the same file: an anchor (`&name`) defined in `blueprint.yaml` cannot be used as an alias (`*name`) in an imported file like `inputs.yaml`. If you need to reuse a block across imported files, duplicate the anchor definition in each file or inline the values directly.
...
node_templates:
  helm_release:
    ...
    properties:
      # ... look up exact properties with: dap-bpa knowledge plugins get helm dell.nodes.helm.Release
    relationships:
      - target: vm
        type: dell.relationships.contained_in
```
...
3. **YAML anchors are file-scoped**: Each imported file that needs anchors must define its own `dsl_definitions` block. See the callout above (`inputs.yaml` section) for the full explanation and fix options.
...
1. `check_drift` — Compare live state to desired state and report the result via `ctx.returns(<payload>)`. Must be **read-only**: do not mutate `runtime_properties` or `system_properties`.
...
- Report the result via **`ctx.returns(<payload>)`** — this is the only channel the orchestrator reads; a plain Python `return` is ignored. Recommended payload shape: `{resource_id, diff_count, diff, state_drift, time}` (unlocks the "View actual drift" UI). Minimum acceptable: `{'drift': True | False}`. Return `None` when there is no drift.
- Be **read-only** — `check_drift` is invoked during `preview` mode, so it must not mutate `runtime_properties` or `system_properties`.

See the canonical runnable reference at `blueprints/script/drift-example-blueprint.yaml` (in the blueprint knowledge base) for a full `check_drift.py` built against this contract.
