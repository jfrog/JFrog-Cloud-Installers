...
All commands output JSON. Use `dap-bpa <command>` if installed, or `./bpa-docker.sh <command>` via Docker if not.

Commands follow a hierarchical form: `dap-bpa <group> <subgroup> <action>`.

---

## CLI Availability

1. **Direct CLI**: `dap-bpa` — Installed via npm or locally built
2. **Docker runner**: `bpa-docker.sh` — Runs via Docker container when CLI not installed

```bash
# If dap-bpa is installed (npm or local build)
dap-bpa <command> [options]

# If dap-bpa is not available, use Docker runner
./bpa-docker.sh <command> [options]
```

**When to use which:**
- Use `dap-bpa` if it's installed (fastest, no Docker overhead)
- Use `bpa-docker.sh` if `dap-bpa` command is not available (look for the script in `~/tmp/` or project directory)
...
**Always attempt the command immediately.** Do not ask the user whether credentials are configured, and do not explain auth requirements upfront. Just run the command and report the actual output. If credentials are missing, `dap-bpa` will return a clear error — only then explain what needs to be set up.
...
1. **Search for an example** — always start here, never from scratch:
   ```bash
   dap-bpa knowledge blueprints find "<what the user wants>"
   ```
...
8. If linting or validation finds errors, fix them and validate again.
   ...
   **If you believe a lint warning is a false positive**, re-run with `--report-fp`:
   ```bash
   dap-bpa blueprint lint --file <path> --report-fp
   ```
...
```bash
dap-bpa orchestrator blueprints upload --file blueprint.yaml --id <blueprint_id> --revision 1.0.0
```
...
```bash
dap-bpa orchestrator blueprints upload --file my-blueprint.zip --id <blueprint_id> --revision 1.0.0
```
...
```bash
dap-bpa orchestrator blueprints upload --file my-blueprint.zip --id <blueprint_id> --revision 1.0.0 --application-file-name my-blueprint.yaml
```
...
dap-bpa orchestrator blueprints upload --file my-blueprint.zip --id "my-blueprint" --revision 1.0.0

# If your main YAML has a non-standard name, specify it explicitly
dap-bpa orchestrator blueprints upload --file my-blueprint.zip --id "my-blueprint" --revision 1.0.0 --application-file-name my-blueprint.yaml
```
...
```bash
dap-bpa orchestrator blueprints upload --file blueprint.yaml --id <blueprint_id> --revision 1.0.0
dap-bpa orchestrator blueprints get <blueprint_id> --fields id state
...
1. **Check the current version:**
   ```bash
   dap-bpa orchestrator blueprints get <blueprint_id> --fields id revisions
   ```
...
3. **Upload with the user-specified revision:**
   ```bash
   dap-bpa orchestrator blueprints upload --file blueprint.yaml --id <blueprint_id> --revision <new_version>
   ```

4. **Confirm completion:**
   ```bash
   dap-bpa orchestrator blueprints get <blueprint_id> --fields id state revisions
   ```
...
```bash
dap-bpa orchestrator deployments create --blueprint-id <blueprint_id> [--inputs <json-file>] [--display-name <name>] [--environment <env-id>]
```
...
Then reference it:
```bash
dap-bpa orchestrator deployments create --blueprint-id my-blueprint --inputs deployment-inputs.yaml
```

**Direct JSON input:**
```bash
dap-bpa orchestrator deployments create --blueprint-id my-blueprint --inputs '{"vm_name": "my-server", "cpu_count": 2}'
```
...
1. `dap-bpa orchestrator blueprints get <id> --fields id description requirements revisions`
2. Look at `requirements.secrets` to identify needed secrets
3. `dap-bpa orchestrator secrets list` — verify all required secrets exist
...
1. **Add plugin to knowledge base:**
   ```bash
   dap-bpa knowledge plugins add helm --search-path ~/plugins
   ```
...
If `dap-bpa knowledge plugins docs <plugin>` returns `{ "error": "...", "available": [...] }` and the plugin name you need is not in the `available` list, the docs for that plugin haven't been fetched locally yet.

Use `dap-bpa knowledge plugins fetch` to pull them from the upstream repo:

```bash
# Fetch docs for a single plugin
dap-bpa knowledge plugins fetch kubernetes
...
dap-bpa orchestrator blueprints list --fields id name state
dap-bpa orchestrator blueprints get <blueprint_id>
dap-bpa orchestrator blueprints get gs-1 --fields id description requirements
dap-bpa orchestrator blueprints upload --file <path>
dap-bpa orchestrator blueprints upload --file <path> --id <id> --revision <ver>
dap-bpa orchestrator blueprints upload --file <archive.zip> --id <id> --revision <ver> --application-file-name <name.yaml>
```
...
dap-bpa orchestrator deployments get d5e3f0e1-5670-4667-9b41-337babf3b5a2
dap-bpa orchestrator deployments create ...
dap-bpa orchestrator deployments update <id> --inputs <json-file>
...
> **Note**: `dap-bpa orchestrator deployments update` (above) only PATCHes the inputs field directly. Use `deployment-updates initiate` for the full deployment update workflow with lifecycle operations.

```bash
dap-bpa orchestrator deployment-updates initiate <deployment_id> --body <body.json>
dap-bpa orchestrator deployment-updates list
dap-bpa orchestrator deployment-updates list <deployment_id>
dap-bpa orchestrator deployment-updates get <update_id>
```
...
dap-bpa orchestrator executions get b319198a-d395-4587-9138-5e7c530f409f --fields id status error parameters
...
dap-bpa orchestrator plugins get cc54f9eb-d537-46b9-b526-a34803ae5562
dap-bpa orchestrator plugins upload --file <path.wgn|.zip> [--name <display-name>] [--visibility tenant|global]
...
```bash
dap-bpa orchestrator events list
dap-bpa orchestrator events list --limit 50
dap-bpa orchestrator events list --from-dt 2026-03-01T00:00:00Z --to-dt 2026-03-02T00:00:00Z

dap-bpa orchestrator events get <execution_id>
dap-bpa orchestrator events get b319198a-d395-4587-9138-5e7c530f409f --batch-size 50
```
...
dap-bpa orchestrator secrets get <name>
```
...
dap-bpa knowledge plugins list openstack
dap-bpa knowledge plugins list kubernetes
dap-bpa knowledge plugins list edge

dap-bpa knowledge plugins get helm dell.nodes.helm.Release
dap-bpa knowledge plugins get openstack dell.nodes.openstack.Server
```
...
dap-bpa knowledge types get dell.nodes.vsphere.Server
dap-bpa knowledge types get dell.nodes.ServiceComponent
dap-bpa knowledge types get dell.relationships.depends_on

# List all known types (base + plugin)
dap-bpa knowledge types list
dap-bpa knowledge types list --source base
dap-bpa knowledge types list --source plugins
```
...
```bash
dap-bpa knowledge docs search <query> [--plugin x] [--limit n]
dap-bpa knowledge docs find <query>    # alias
```
...
Examples:
```bash
dap-bpa knowledge docs search "aws authentication"
dap-bpa knowledge docs search "security groups" --plugin aws
dap-bpa knowledge docs search "k8s auth" --limit 10
```

**Get a specific doc** (direct file access):
```bash
dap-bpa knowledge docs get <path>
```
...
Examples:
```bash
dap-bpa knowledge docs get aws/plugin
dap-bpa knowledge docs get general/k8s-auth
dap-bpa knowledge docs get vsphere/plugin
```
...
```bash
dap-bpa knowledge plugins docs <plugin>
dap-bpa knowledge plugins node-type-docs <plugin> <node_type>
```
...
{ "error": "Docs for '<name>' not found", "available": ["ansible", "aws", "helm", ...] }
```
Use the `available` array to find the correct name or confirm whether the plugin docs have been fetched at all. If the plugin is absent from `available`, fetch it with `dap-bpa knowledge plugins fetch <plugin>`.

### Blueprint Examples

```bash
# Query can be positional argument OR --query flag
dap-bpa knowledge blueprints find "deploy vm in vsphere"
dap-bpa knowledge blueprints find --query "kubernetes" --plugin kubernetes
dap-bpa knowledge blueprints find "helm chart" --type single

dap-bpa knowledge blueprints get single-vsphere
dap-bpa knowledge blueprints get multi-k8s --include-files

# Import a single blueprint directory into the local library
dap-bpa knowledge blueprints add ./my-blueprint

# Import from an archive
dap-bpa knowledge blueprints add ./my-blueprint.zip

# Recursively import all blueprints under a root directory
dap-bpa knowledge blueprints add ~/my-projects

# Preview what would be imported without writing
dap-bpa knowledge blueprints add ~/my-projects --dry-run

# Override depth and file-type defaults
dap-bpa knowledge blueprints add ~/my-projects --scan-depth 3 --copy-depth 2

# Recognise non-standard main filenames
dap-bpa knowledge blueprints add ~/my-projects --main-file-names "blueprint.yaml main.yaml"

# Skip confirmation prompt
dap-bpa knowledge blueprints add ~/my-projects --yes

# Deprecated — use add instead:
# dap-bpa knowledge blueprints scan <dir>
# dap-bpa knowledge blueprints registry <file>
```

### Blueprint Linting

```bash
dap-bpa blueprint lint --file path/to/blueprint.yaml
dap-bpa blueprint lint --file path/to/blueprint.yaml --verify
dap-bpa blueprint lint --file path/to/blueprint.yaml --report-fp
dap-bpa blueprint lint --content "tosca_definitions_version: dell_1_1"
```
...
```bash
dap-bpa blueprint validate <node> --file path/to/blueprint.yaml
dap-bpa blueprint validate-all --file path/to/blueprint.yaml
```

### Adding Plugins to Knowledge Base

```bash
dap-bpa knowledge plugins add <name>
dap-bpa knowledge plugins add helm --search-path ~/plugins
```
...
```bash
dap-bpa knowledge plugins fetch <plugin>
dap-bpa knowledge plugins fetch --all
dap-bpa knowledge plugins fetch helm --branch develop
dap-bpa knowledge plugins fetch helm --upstream https://github.com/your-org/your-fork
```
