...
This skill provides a blueprint authoring reference distilled from the official Blueprint Developer's Guide (Feb 2026, Rev A01), plus {{RUNTIME_INTRO}}.
...
{{COMBINED_SEARCH_BLOCK}}

### 2. Targeted research

{{TARGETED_RESEARCH_BLOCK}}
...
Lifecycle completeness (ND-003): Every node with `create` MUST define `delete`. Every node with `start` MUST define `stop`. Before writing lifecycle operations, load the full reference: {{LIFECYCLE_DOCS_REF}}

Never invent node types. Only use types returned by {{NODE_TYPE_GUARD_LOOKUP}}. If the type is not in the result, it does not exist.
...
For plugin-specific relationship types, `run_on_host`, and decision guidance, see `knowledge/docs/general/relationships.md`. For lifecycle operation patterns (ND-003, ND-004), load {{LIFECYCLE_DOCS_REF}}.
