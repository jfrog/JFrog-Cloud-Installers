...
All deployment update operations use function tools from `@blueprint-assist/core/skill-functions`. Do not execute shell commands.

> **Important distinction**: `updateDeployment` patches deployment metadata only (display name, tags). It is NOT a deployment update workflow. Use `startExecution` with `workflowId: "update"` to trigger the full deployment update workflow.

### Initiate a deployment update

There is no dedicated `initiateDeploymentUpdate` function. Use `startExecution` with the `update` workflow and pass parameters matching the request body:

```ts
const credentials = getCredentials();

// Inputs-only update
const execution = await startExecution(credentials, "my-deployment", {
  workflowId: "update",
  parameters: {
...
### List and get deployment updates

There is no dedicated deployment-updates list/get function. Use `listExecutions` filtered by workflowId:

```ts
const result = await listExecutions(credentials, "my-deployment", {
  workflowId: "update",
});
```
...
After initiating, use the returned `execution.id` to track progress:

```ts
const execution = await getExecution(credentials, executionId);
// execution.status, execution.finishedOperations, execution.totalOperations, execution.error
```

For events, use a host-provided events tool if available, or poll `getExecution` for status.
...
// 1. Upload the new blueprint revision — use the orchestrator UI or direct REST API
//    (no uploadBlueprint function available in skill-functions yet)

// 2. Initiate the deployment update
const execution = await startExecution(credentials, "my-deployment", {
  workflowId: "update",
  parameters: { blueprint_id: "my-bp", blueprint_version: "v2.1.0", skip_reinstall: true },
});

// 3. Monitor progress
const status = await getExecution(credentials, execution.id);
```
...
- **`updateDeployment()` is NOT this** — that function patches deployment metadata only (display name, tags). Use `startExecution(credentials, deploymentId, { workflowId: "update", parameters: { ... } })` for the full update workflow.
...
```ts
findBlueprintExamples({ query: 'deployment update' })
findBlueprintExamples({ query: 'skip reinstall' })
```
