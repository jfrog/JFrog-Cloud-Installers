...
The validator understands all four blueprint kinds (top-level orchestrator, helm component, kubernetes utility, central-agent utility) and applies the helm-chain check only to helm-typed blueprints. It also enforces the catalog label scheme — `csys-obj-type` under `labels`, `env` under `blueprint_labels`, and `csys-blueprint-type` on every non-orchestrator (utility/helm) blueprint.

The Python scripts under `blueprints/*/application/scripts/` are kept clean under open-source static analysis — `bandit` (no security findings), `ruff` (no bugs/code-smell findings beyond intentional line-length), and `yamllint` (using `.yamllint`) on the blueprint YAML.
