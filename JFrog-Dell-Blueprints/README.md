...
- For licensed deployments: a DAP general secret containing the raw Artifactory license text. **Required** when `sizing_template` is `medium`, `large`, `xlarge`, or `2xlarge` (Enterprise license, one per replica) — the input validator fails fast when `create_license_secret="false"` or `license_secret_ref` is unset for those tiers.
