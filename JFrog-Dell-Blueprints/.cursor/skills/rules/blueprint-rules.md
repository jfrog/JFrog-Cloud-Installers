...
| ND-009 | Enabled | Nodes should implement lifecycle operations for the `update` workflow. | Optional | For full update support, the following sub-operations are recommended: `check_drift`, `preupdate`, `update`, `postupdate`, `update_config`, `update_apply`, `update_postapply`, `preheal`, `heal`, `postheal`. | |
