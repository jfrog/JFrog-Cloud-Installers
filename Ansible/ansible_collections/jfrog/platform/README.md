...
If an external database for one or more products is to be used, omit the `postgres` role from `platform.yml` and set `postgres_enabled: false` in `group_vars/all/vars.yml`.

Create an external database as documented in [Creating the Artifactory PostgreSQL Database](https://www.jfrog.com/confluence/display/JFROG/PostgreSQL#PostgreSQL-CreatingtheArtifactoryPostgreSQLDatabase) and change the corresponding product values in `group_vars/all/vars.yml`.

For example, for artifactory, the following values need to be set for using external postgresql
...
* Centos/RHEL 8.x/9.x; RHEL 10.x (except Xray)
* Debian 11.x; Debian 13.x (except Xray)
* Amazon Linux 3
* SLES 15
